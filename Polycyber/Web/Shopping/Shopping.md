...
We are given downloadable source file for the web application, so we can take a look at it to try to understand what it's doing a bit better

![Alt text](Images/downloads.png)
  
# Source code analysis:
## signup.php
First, when reading the signup.php page we find a SQLi vulnerability showcased here:
![Alt text](Images/signup_sql.png)
But that doesn't seem of much help currently, so we keep on reading..
## addproduct.php

We can find an obvious LFI vulnerability in the `role` parameter here, which seems interesting
![Alt text](Images/lfi.png)
So maybe we can just pass /flag or /flag.txt to it? Or maybe not...
After numerous tries to bruteforce the flag location, it becomes obvious that we need to obtain RCE to be able to find the flag, and then read it. So I start looking into ways to escalate our LFI to RCE and a couple of ways come to mind instantly:
- Log poisoning
- PHP Filter chains
and so we start...
...
------------------------------------------------------------------------------
We start trying the different techniques above to obtain RCE, guided by our lovely [hacktricks](https://book.hacktricks.xyz/pentesting-web/file-inclusion)
## Log poisoning:

That's usually a low-hanging fruit in CTF challenges, and so I didn't go in expecting it to be log poisoning, but I tried anyway, but to no avail.

## PHP Filter chains:

This seems to be a technique that's used alot in the CTF community, but I can't blame them because I really like this vulnerability.
**If you're not sure what that means, you can watch [this](https://www.youtube.com/watch?v=PVdOSpF4Tl0) video by `Tib3rius`, which is a more than enough explanation to walk away with heaps of knowledge.**

And while doing research to refresh my memory, I passed upon a script which I used to solve a box on HackTheBox a while back, and it was very helpful then, so it must be now! The script can be found [here](https://gist.github.com/loknop/b27422d355ea1fd0d90d6dbc1e278d4d)

And so after some minor modifications we have a running script, so we can use it to obtain the flag!
...
r = requests.get(url, params={               # GET THE CONTENTS OF THE /opt FOLDER
    "0": command,
    "action": "include",
    "role": final_payload
})
...
print(r.text)
command = "cat /opt/4c9ef90fdcdd387456f56cd2d275a7f5"                   # READ THE FLAG FILE
